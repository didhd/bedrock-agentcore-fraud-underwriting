...
def _deployed_config() -> dict:
    """`agentcore/agentcore.json`, or {} when it cannot be read.

    This is the file `agentcore deploy` acts on, so it is the truthful answer to "what will
    the deployed runtime see". It matters most for the BAKED api objects that CloudFront
    serves: those are generated by a build process whose shell environment is not the
    runtime's, so a panel that read `os.environ` reported "not configured" beside a live
    cluster. Reading the deploy config instead describes the thing the reader is looking at.
    """
    try:
...
def _gateway_tools() -> list[dict]:
    """Tools the deployed AgentCore Gateway exposes, from the config and its schema files.

    Read from `agentcore.json` and the `toolSchemaFile` each target names, because those are
    what the deploy declares -- and the names are mechanical: the Gateway prefixes every tool
    with `<TargetName>___`. Verified against a live Francis, which listed exactly these.

    These are NOT subject to her two-specialist cap. `ToolCapHook` counts only the eight
    specialist tools, deliberately: the customer's "MAXIMUM 2 tools per query" is a rule about
    fan-out, written for latency, and a dictionary lookup is not fan-out.
    """
    import json as _json  # noqa: PLC0415
    import pathlib as _pathlib  # noqa: PLC0415
    ...
    out: list[dict] = []
    for gateway in _deployed_config().get("agentCoreGateways", []) or []:
        for target in gateway.get("targets", []) or []:
            block = target.get("lambdaFunctionArn") or {}
            schema_file = block.get("toolSchemaFile")
            if not schema_file:
                continue
            try:
                schema = _json.loads((root / schema_file).read_text())
            except Exception:
                continue
            for tool in schema:
                out.append(
                    {
                        # The Gateway's own naming. Getting this wrong is silent: the model
                        # calls a name that returns unknown-tool.
                        "name": f"{target.get('name')}___{tool.get('name')}",
                        "via": f"gateway:{gateway.get('name')}",
                        "counts_toward_cap": False,
                        "summary": (tool.get("description") or "").split(". ")[0][:180],
                    }
                )
    return out
...
def _tools_of(agent_id: str) -> dict[str, Any]:
    ...
    if agent_id == "francis":
        try:
            ...
            gateway = _gateway_tools()
            return {
                "count": len(DOMAINS) + len(gateway),
                "specialist_count": len(DOMAINS),
                "max_per_query": 2,
                "why_capped": (
                ...
                # The Gateway tools are hers too, and omitting them under-reported her real
                # surface: a live Francis lists these alongside the eight. They are declared
                # statically in agentcore.json, so this is read rather than guessed.
                + gateway,
                "gateway_tool_count": len(gateway),
            }
        except Exception:
