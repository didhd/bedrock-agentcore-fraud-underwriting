...
import type { ApplicationSummary, Health, PayloadResponse } from "@/lib/types"
    ...
    <header className="flex flex-wrap items-start justify-between gap-4 border-b border-border pb-5">
      <div className="flex flex-col gap-1.5">
        ...
        <p className="max-w-3xl text-sm text-muted-foreground">
          Eight specialist fraud agents and one master synthesizer, running the customer's own
          production prompts byte-for-byte. Each specialist is dispatched in parallel on its own
          right-sized model; the synthesizer returns the {health?.adjudication_keys.length ?? 21}-key
          adjudication under its existing contract.
        </p>
      </div>
      ...
      <AlertDescription>
        <div className="flex flex-col gap-1">
          <span className="text-xs">
            This process reported the following about what is actually wired. Panels whose source
            module has not landed say so rather than showing a substitute number.
          </span>
...
function Demo() {
  ...
  const [backend, setBackend] = React.useState<BackendConfig>({ kind: "local" })
  ...
  React.useEffect(() => {
    getJson<Health>("/api/health")
      .then(setHealth)
      .catch((error: Error) => toast.error(`Could not load /api/health: ${error.message}`))
  }, [])
  ...
  React.useEffect(() => {
    getJson<ApplicationSummary[]>("/api/applications")
      ...
      .catch((error: Error) => toast.error(`Could not load /api/applications: ${error.message}`))
  ...
  React.useEffect(() => {
    if (state.errors.length > seenErrors.current) {
      state.errors.slice(seenErrors.current).forEach((error) => toast.error(error))
  ...
  const run = React.useCallback(
    (applicationId: string) => {
      if (backend.kind === "agentcore" && !backend.endpoint) {
        toast.error("Enter the AgentCore Runtime invocation URL before running.")
        return
      }
  ...
  return (
    <div className="mx-auto flex max-w-[92rem] flex-col gap-5 px-5 py-6">
      ...
      <BackendBar
        ...
        applications={applications.map((application) => application.application_id)}
        selected={selected}
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        <SignalLayerPanel payload={payload} loading={loadingPayload} applicationId={selected} />
        <CostPanel
          cards={cards}
          synthesis={state.synthesis}
          measuredTokens={state.measuredTokens ?? health?.measures_tokens ?? null}
        />
      </div>

      <footer className="border-t border-border pt-4 pb-2 text-xs text-muted-foreground">
        <p>
          Every measured value on this page came from this run. A field that was not measured shows
          an em dash rather than a placeholder.
          {state.executionPath ? (
            <>
              {" "}
              Execution path: <span className="font-mono">{state.executionPath}</span>.
            </>
          ) : null}
          {state.sessionId ? (
            <>
