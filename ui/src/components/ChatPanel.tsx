 ...
 * token by token, and holds session context. A specialist is not a chatbot -- its prompt
 * makes it analyse one domain of one application -- so its seat takes free text, finds
 * the application id in it, and returns that domain's analysis in one piece. Presenting
 * a specialist as conversational would misrepresent the customer's own design, so the
 * placeholder and the badge say which kind of seat you are in.
 *
...
export function ChatPanel({ seats }: { seats: AgentSeat[] }) {
      ...
      <CardHeader className="gap-2">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <CardDescription>
              Every reply comes from a deployed AgentCore runtime. Nothing on this tab is
              simulated; a seat whose runtime is not deployed refuses rather than answering.
            </CardDescription>
          </div>
        ...
        {seat ? (
          <div className="flex flex-wrap items-center gap-2 text-xs">
            <Badge variant={seat.conversational ? "default" : "secondary"} className="font-mono text-[0.625rem]">
              {seat.conversational ? "conversation" : "single-domain analysis"}
            </Badge>
            {seat.runtime ? (
              <Tooltip>
                <TooltipTrigger asChild>
                  <Badge variant="outline" className="cursor-help font-mono text-[0.625rem]">
      ...
      <CardContent className="flex min-h-0 flex-1 flex-col gap-3 pt-4">
            ...
            <div className="flex flex-col gap-2 py-10 text-center text-sm text-muted-foreground">
              <p>
                {seat?.conversational
                  ...
                  : "Mention an application id and this specialist analyses its own domain."}
              </p>
              <p className="font-mono text-xs">
                {seat?.conversational
                  ? '"Is there income fraud on APP-1003?"'
                  : '"What is the risk on APP-1004?"'}
              </p>
            </div>
          ) : null}
                ...
                <div className="flex min-w-0 flex-1 flex-col gap-1.5">
                  ...
                  {turn.tools?.length ? (
                    <div className="flex flex-wrap items-center gap-1.5">
                      {turn.tools.map((tool) => (
                        <Badge key={tool} variant="secondary" className="font-mono text-[0.625rem]">
                  ...
                  {turn.usage && !turn.streaming ? (
                    <div className="flex flex-wrap gap-x-3 gap-y-1 font-mono text-[0.625rem] text-muted-foreground">
                      {turn.usage.risk_band ? <span>{turn.usage.risk_band}</span> : null}
        ...
        <div className="flex items-end gap-2">
          <textarea
            ...
            placeholder={
              seat?.conversational
                ? "Ask Francis about an application, a lender, or a trend..."
                : "Mention an application id, e.g. APP-1004..."
            }
          ...
          />
          <Button onClick={() => void send()} disabled={busy || !input.trim() || !seat?.runtime}>
            {busy ? (
