...
function handler(event) {
  ...
  return request;
    ...
    const streamProxy = new lambda.Function(this, 'StreamProxy', {
      description:
        'Signs and streams one AgentCore underwriting run to the browser, translating ' +
        "the runtime's SSE vocabulary into the one ui/src/hooks/useRun.ts consumes",
      runtime: lambda.Runtime.NODEJS_22_X,
      handler: 'stream-proxy.handler',
      code: lambda.Code.fromAsset(path.join(__dirname, '..', 'lambda'), {
        // bootstrap.json is generated, so it is copied in at synth time rather than
        // committed next to the handler.
        bundling: undefined,
      }),
      ...
      // degraded run without ever approaching AgentCore's own 60s streaming limit
      // being the thing that decides the outcome.
      timeout: Duration.minutes(5),
      // The work is one signed POST and then piping bytes; it is entirely IO-bound.
    ...
    // Scoped to the one runtime this stack fronts, not to `runtime/*`. The account
    // has 39 other runtimes and none of them should be reachable from a public URL.
